...
public class GunFactoryMenu extends Menu {
    private final Building b = menuUtility.getBuilding();
    private int unitLvl = (int) b.getOwner().getAttribute(EPlayerAttribute.ART_LVL);
    private UnitTech unitTech = UnitTech.valueOf("ART"+unitLvl);
    private ItemStack buildingItem = menuUtility.getBuildingItem();
    private Map<Material,Integer> materials = unitTech.getMaterials();
    private final ServerDatabase db = Earth.getInstance().getDatabase();

    public GunFactoryMenu(MenuUtility menuUtility) {
        super(menuUtility);
    }
                ...
                switch (data.get(menuIdKey, PersistentDataType.STRING)){
                    ...
                    case "merc" ->{
                        if(
                                ...
                                && army!=null
                                && b.getOwner().getAttribute(EPlayerAttribute.TREASURY)>=162
                        ){
                            ArmyUnit unit = new ArmyUnit(unitTech,UUID.randomUUID(),army.getUuid(),"");
                            b.getOwner().addAttribute(EPlayerAttribute.TREASURY,-162);
                    ...
                    case "army" ->{
                        ...
                        e.getWhoClicked().addPotionEffect(slowness);
                        e.getWhoClicked().addPotionEffect(glowing);
    ...
    public void setMenuItems() {
        ...
        sLore.add("Стоимость: <gold>162<white>$");
        sLore.add("Содержание: "+ (3.5 + unitLvl) + "$");
        sLore.add("Лимит: " + b.getOwner().getMercAmount() + "/" + b.getOwner().getManpowerLimit()/1000);
        ItemStack merc = makeItem("Нанять наёмников " + unitLvl + " уровня","merc","art",sLore);
        ...
        sLore = new ArrayList<>();
        sLore.add("Стоимость:");
        sLore.add("<gold>81 <white>$");
        sLore.addAll(CustomConfig.get().getStringList("tech.lore.art"+unitLvl));
        sLore.add("Содержание: 3.5$");
        ItemStack regular = makeItem("Нанять артиллерию " + unitLvl + " уровня","regular","art",sLore);
        ...
        if(army==null){
            ...
            armyShulker = makeItem(Material.SHULKER_BOX,"Отсутствует армия","create","cav",sLore);
            ...
            armyShulker = makeItem(Material.SHULKER_BOX,"Армия " +army.getOwner().getDisplayName(),"army","cav",sLore);
        }
        ...
        inventory.setItem(4, buildingItem.clone());
    ...
    private boolean tryPayment(Player player, Map<Material,Integer> requirements) {
        ...
        if(b.getOwner().getAttribute(EPlayerAttribute.TREASURY)<81){
            return false;
        }
        ...
        b.getOwner().addAttribute(EPlayerAttribute.TREASURY,-81);
