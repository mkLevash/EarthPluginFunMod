...
public class StableMenu extends Menu {
    ...
    private UnitTech unitTech = UnitTech.valueOf("CAV"+(int)b.getOwner().getAttribute(EPlayerAttribute.CAV_LVL));
    private int unitLvl = (int) b.getOwner().getAttribute(EPlayerAttribute.CAV_LVL);
    ...
    public StableMenu(MenuUtility menuUtility) {
        super(menuUtility);
    }
        ...
        if (rawSlot < topSize) {
            ...
            if(data.has(menuIdKey)){
                switch (data.get(menuIdKey, PersistentDataType.STRING)){
                    ...
                    case "merc" ->{
                        if(b.getOwner().getMercAmount()<b.getOwner().getManpowerLimit()/1000&&army!=null&&b.getOwner().getAttribute(EPlayerAttribute.TREASURY)>=36){
                            ArmyUnit unit = new ArmyUnit(unitTech,UUID.randomUUID(),army.getUuid(),"");
                            b.getOwner().addAttribute(EPlayerAttribute.TREASURY,-36);
    ...
    public void setMenuItems() {
        ...
        sLore.add("Стоимость: <gold>36<white>$");
        sLore.add("Содержание: "+ (2 + unitLvl) + "$");
        sLore.add("Лимит: " + b.getOwner().getMercAmount() + "/" + b.getOwner().getManpowerLimit()/1000);
        ItemStack merc = makeItem("Нанять наёмников " + unitLvl + " уровня","merc","cav",sLore);
        ...
        sLore = new ArrayList<>();
        sLore.add("Стоимость:");
        sLore.add("<gold>18 <white>$");
        sLore.addAll(CustomConfig.get().getStringList("tech.lore.cav"+unitLvl));
        sLore.add("Содержание: 2$");
        ItemStack regular = makeItem("Нанять кавалерию " + unitLvl + " уровня","regular","cav",sLore);
    ...
    private boolean tryPayment(Player player, Map<Material,Integer> requirements) {
        ...
        if(b.getOwner().getAttribute(EPlayerAttribute.TREASURY)<18){
            return false;
        }
